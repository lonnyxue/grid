...
public class DEGridV2AddMoney {
    ...
    private static final double ADD_MONEY_PERCENT;
    ...
    static {
        ...
        ADD_MONEY_PERCENT = setting.getDouble("add_money_percent");
    }

    public static void main(String[] args) {
        DEGridV2AddMoney grid = new DEGridV2AddMoney();
        grid.grid();
...
//        gridModels.addAll(genPricesLte());
    ...
    private List<GridModel> genPricesGt() {
        ...
        do {
            ...
            level++;
            gridModels.add(createOneGrid(nextBuyPrice, nextSellPrice, gridBuyLevel, BUY_AMOUNT));
    ...
    private List<GridModel> genPricesLte() {
        ...
        for (int i = 0; i <= grids; i++) {
            ...
            if (ADD_MONEY_PERCENT > 0 && i > 0) {
                curBuyPlace = BUY_AMOUNT + (BUY_AMOUNT * i * ADD_MONEY_PERCENT / 100);
            }
            gridModels.add(createOneGrid(buyPrice, sellPrice, gridBuyLevel, curBuyPlace));
        }
        return gridModels;
    }
    ...
    private GridModel createOneGrid(double buyPrice, double sellPrice, double buyLevel, double buyPriceSum) {
        GridModel gridModel = new GridModel();
        gridModel.setLevel(buyLevel);
        gridModel.setBuyPrice(buyPrice);
        gridModel.setBuyNum((int) Math.round(buyPriceSum / buyPrice));
        gridModel.setBuyPriceSum(buyPriceSum);
